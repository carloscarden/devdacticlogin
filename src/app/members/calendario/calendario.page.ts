...
export class CalendarioPage implements OnInit {
  ...
  nombreMeses = ['ENERO', 'FEBRERO', 'MARZO', 'ABRIL', 'MAYO',
  ...
  ngOnInit() {
    this.page = parseInt(this.mesAbuscar, 10) + 1;
    this.maximumPages = 1;
  ...
  nuevoMes(mes, infiniteScroll?) {
    ...
    this.agendaService.getEvents(mes, this.anio, this.inspectorId)
      .subscribe(res => {
        const mesActual = this.nombreMeses[mes - 1];
        console.log(res);
        if (res == null) {
  ...
  changeMode(mode) {
    const url = '/members/menu/agenda/' + mode;
    this.router.navigateByUrl(url);
  }

  buscar(mes, nuevaTarea) {
    for (let i = 0; i < this.mesesCargados.length; i++) {
