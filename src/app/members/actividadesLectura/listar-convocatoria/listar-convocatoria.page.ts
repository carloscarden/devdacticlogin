...
export class ListarConvocatoriaPage implements OnInit {
  ...
  loadConvocatorias(page, infiniteScroll?) {
    ...
    if (page <= this.maximumPages) {
      ...
      if (this.filtroActivado) {
        const usrQuiereFiltroFecha = this.usuarioQuiereFiltrarPorFecha();
  ...
  cargarConvocatoriasDesdeHasta(page, infiniteScroll?) {
    ...
    const diaInicio = this.inicioFiltro.split('-');
    const formatoCorrectoInicio = diaInicio[0] + '/' + diaInicio[1] + '/' + diaInicio[2];

    const diaFin = this.finFiltro.split('-');
    const formatoCorrectoFin = diaFin[0] + '/' + diaFin[1] + '/' + diaFin[2];
    ...
    if (this.tipoFiltro === 'Todos.') {
      ...
      this.convocatoriaService.getConvocatoriasByArticuloAndDate
        (this.size, page, this.idInspector, formatoCorrectoInicio, formatoCorrectoFin, this.tipoFiltro)
        .subscribe(res => {
  ...
  crearPDF() {
    ...
    const contenidoDelPDF = this.diseniarPDFcomoLoQuiereElUsuario();
    contenidoDelPDF.subscribe(res => {
      if (res != null) {
        ...
        const contenidoArmadoParaElPDF = this.armarPDF(convocatoriasAllenar);
  ...
  armarPDF(convocatoriasAllenar: Array<Convocatoria>) {
    ...
    const contenidoArmadoDelPDF = [];
    const currentUser = this.authenticationService.currentUserValue;
    const inspector = currentUser.nombre + ' ' + currentUser.apellido;
    const diaDeHoy = new Date();
    const diaDeHoyFormatoPDF = diaDeHoy.getDate() + '/' + (diaDeHoy.getMonth() + 1) + '/' + diaDeHoy.getFullYear();

    convocatoriasAllenar.forEach(convocatoria => {
      console.log(convocatoria);
      const contenidoDeLaLicencia = [];
      const formatoInicio = this.formatoHoraPDF(convocatoria.inicio);
      const formatoFin = this.formatoHoraPDF(convocatoria.fin);
      contenidoDeLaLicencia.push
        (formatoInicio, formatoFin, convocatoria.tipoConvocatoria.descripcion, convocatoria.distrito.descripcion, convocatoria.lugar);
  ...
  abrirYdescargarPDF(contenidoArmadoParaElPDF) {
    const pdfObj = pdfMake.createPdf(contenidoArmadoParaElPDF);
  ...
  stringAsDate(dateStr) {
    const reemplazar = dateStr.replace(/-/g, '/');
  ...
  validarFechas() {
    if (this.inicio != null) {
      if (this.fin != null) {
        const a = this.inicio.split('-');
        const inicioSinHoras = new Date(parseInt(a[2], 10), parseInt(a[1], 10) - 1, parseInt(a[0], 10));
        console.log('inicio a validar', inicioSinHoras);

        const b = this.fin.split('-');
        const finSinHoras = new Date(parseInt(b[2], 10), parseInt(b[1], 10) - 1, parseInt(b[0], 10));
  ...
  usuarioQuiereFiltrarPorFecha() {
    ...
    const fechasVacias = (this.inicioFiltro == null || this.finFiltro == null) || (this.inicioFiltro === '' || this.finFiltro === '');
  ...
  formatoHoraPDF(diaYhora) {
    const s = diaYhora.split(' ');
    const dia = s[0].split('-');
    const hora = s[1];
    const fecha = dia[1] + '/' + dia[0] + '/' + dia[2] + ' ' + hora;
    return fecha;
  }
