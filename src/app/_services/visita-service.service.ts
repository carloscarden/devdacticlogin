...
export class VisitaServiceService {
  ...
  getAllVisitas(idInspector, inicio, fin, motivo, size, page) {
    if (motivo == null && inicio == null && fin == null) {
      ...
      (URL + `inspectores/${idInspector}/visitas?from=${inicio}&to=${fin}&size=${size}&page=${page}&sort=inicio,ASC`);

    } else if (motivo != null && inicio != null && fin != null) {
      // buscar las visitas por articulo inicio y fin
      return this.http.get<any>
      (URL + `inspectores/${idInspector}/visitas?articulo=${motivo}&from=${inicio}&to=${fin}&size=${size}&page=${page}&sort=inicio,ASC`);
  ...
  getVisitasByDateAndMotivo(size, page, idInspector, inicio, fin, motivo): Observable<any> {
    ...
    return this.http.get<any>
    (URL + `inspectores/${idInspector}/visitas?motivo=${motivo}&from=${inicio}&to=${fin}&size=${size}&page=${page}&sort=ASC`);
  }
